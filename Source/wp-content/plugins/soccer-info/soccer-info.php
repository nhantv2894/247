...
if ( !class_exists('SoccerInfo') ) {
	...
	class SoccerInfo {
		...
		public function __construct() {
			...
			define('SOCCER_INFO_VERSION', '1.9.2');
		...
		public function shortcodes_controller($attributes) {
			...
			extract(shortcode_atts(
				array(
					...
					'link'			=> 1, //1 enabled, 0 = no links
					'nofollow'		=> 0 //1 = nofollow added, 0 = default	 //added since 1.9.2 version
                ),
				$attributes
			));
				...
				if ( !is_wp_error( $response ) && isset($response['body']) && !empty($response['body']) ) {
					...
					if ( $league_id < 0 ) {
						...
						if ( $type == 'table' ) {
							...
							$selector = 'table.mx-matches-table';
					}
				...
				if ( is_object($table->tbody) ) {
					...
					foreach ( $table_tbody_0 as $ii => $tr ) {
						...
						if ( $limit == 0 || $i_limit < $limit ) {
							...
							if ( (count($tr->td[0]->div) > 4 + $van_comp) && $enetpulse != false && 
								 trim($tr->td[0]->div[0]->div[2]->span[0]) == 'Finished' ) {
								
								$date_div_attr = $tr->td[0]->div[0]->div[2]->div[0]->attributes();
							...
							if ( ( count($tr->td) > 4 + $van_comp && isset($tr->td[0]->span[0] ) || 
								 ( $enetpulse != false && count($tr->td[0]->div) > 4 + $van_comp && trim($tr->td[0]->div[0]->div[2]->span[0]) == 'Finished' ) ) ) {
								//echo "szilva";
								if ( $enetpulse != false ) {
									$team_a = $this->correct_team_name( trim($tr->td[0]->div[1 + $van_comp]->span[0]->span[0]) );
									$team_b = $this->correct_team_name( trim($tr->td[0]->div[3 + $van_comp]->span[0]->span[0]) );
								}
								else {
								...
								if ( empty($team) || $team == $team_a || $team == $team_b ) { //check for the ONLY team
									...
									$team_b_bold = '';
									if ( $enetpulse != false ) {
										$score = (string)$tr->td[0]->div[2 + $van_comp]->div[0]->span[0].' - '.(string)$tr->td[0]->div[2 + $van_comp]->div[0]->span[1];
									}
									else {
				...
				if ( is_object($table->tbody) ) {
					...
					foreach ( $table_tbody_0 as $ii => $tr ) {
						...
						if ( $limit == 0 || $i_limit < $limit ) {
							...
							if ( (count($tr->td[0]->div) > 4 + $van_comp) && $enetpulse != false && 
								 isset($tr->td[0]->div[0]->div[0]->div[1]->span[0]) && strpos($tr->td[0]->div[0]->div[0]->div[1]->span[0], '/') !== false  ) {
								
								$new_date_formats = explode('-', $tr->td[0]->div[0]->div[0]->div[1]->span[0]);
								if ( count($new_date_formats) == 1 )
								...
								if ( count($new_date_formats) > 1 ) {
										...
										$new_date_formats_m_d = explode('/', $new_date_formats[0]);
										$last_td_date = strtotime($new_date_formats_m_d[1].'/'.$new_date_formats_m_d[0].'/'.$new_date_formats[1]." ".$tr->td[0]->div[0]->div[0]->div[1]->span[2].":00");
							...
							if ( ( count($tr->td) > 4 + $van_comp && isset($tr->td[0]->span[0]) ) || 
								 ( $enetpulse != false && count($tr->td[0]->div) > 4 + $van_comp && strpos($tr->td[0]->div[0]->div[0]->div[1]->span[0], '/') !== false ) ) {
